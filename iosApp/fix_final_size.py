...
def make_icon(source_path, target_path, logo_target_size=924, y_offset=38):
    ...
    # Centering with manual vertical offset to satisfy user's visual preference
    x = (1024 - new_w) // 2
    y = (1024 - new_h) // 2 + y_offset 
    
    # Paste logo onto white background
    final.paste(img, (x, y), img if img.mode == 'RGBA' else None)
    final.save(target_path, "PNG")
    print(f"Icon saved to {target_path} (Final: {new_w}x{new_h}, Y-Offset: {y_offset})")
...
make_icon(source_sita, dest_master, logo_target_size=924)
...
make_icon(source_sita, dest_cardent, logo_target_size=924)
